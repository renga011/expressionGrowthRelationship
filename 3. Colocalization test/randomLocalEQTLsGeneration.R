...
localEQTLs = dplyr::filter(eQTL_Albert2018, cis == "TRUE") %>% select(gene, chr, CILeft, CIRight)

colnames(localEQTLs) = c("variable", "chromosome", "CILeft", "CIRight")

localEQTLs$width = localEQTLs$CIRight - localEQTLs$CILeft + 1

#apply QTL generator to local-EQTL table 

randomLocalEQTLs = parallel::mclapply(randomSet_start:randomSet_end,
                                 FUN = function(randomSet_i){
                                   print(randomSet_i)
                                   rQTLs = mclapply(unique(localEQTLs$variable),
...
#save individual batches of random LocalEQTLs

save(randomLocalEQTLs,
     file = paste0(results_dir, RObj_dir,
                   "randomLocalEQTLs_", batchIndex,".rda"))
